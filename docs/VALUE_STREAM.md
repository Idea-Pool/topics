...
A workshop format is designed to provide an interactive platform (to bring together experts like a value creation network) for problems to be solved by the workshop contributors. Ideally, a workshop is a moderated, offline event (e.g.: brainstorming, white-boarding session) to review the problem and propose solution(s).
This can even be an anonymised example that represent an actual/common challenge on the project (e.g.: an architectural decision or any kind of request for proposal).
